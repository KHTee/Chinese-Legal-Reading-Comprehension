...
class Embedding(nn.Module):
  ...
  def forward(self, c, w):
    ...
    concat_emb = torch.cat(
        (c_emb, emb), 2
    )  # concatenate the word embedding with avg WE and char embedding with avg CE
...
class TriLinearAttention(nn.Module):
  ...
  def forward(self, matrix_1, matrix_2):
    ...
    x_factors = torch.matmul(
        matrix_1.resize(batch_dim * time_1, self.input_dim), self._x_weights)
    x_factors = x_factors.contiguous().view(batch_dim, time_1,
                                            1)  # ->  (batch, time1, 1)

    # (batch * time2, dim) * (dim, 1) -> (batch * tim2, 1)
    y_factors = torch.matmul(
        matrix_2.resize(batch_dim * time_2, self.input_dim), self._y_weights)
    y_factors = y_factors.contiguous().view(batch_dim, 1,
                                            time_2)  # ->  (batch, 1, time2)
...
class SelfAtt(nn.Module):
  ...
  def __init__(self, hidden_size, drop_prob):
    ...
    self.self_att_upsampler = TimeDistributed(
        nn.Linear(hidden_size * 3, hidden_size * 4))
    self.enc = nn.GRU(hidden_size,
                      hidden_size // 2,
                      1,
                      batch_first=True,
                      bidirectional=True)
  ...
  def forward(self, att, c_mask):
    ...
    att_copy = att.clone(
    )  # To save the original data of attention from pervious layer.
    #(batch_size * c_len, 1600)
    att_wrapped = self.att_wrapper(
        att
    )  # unroll the second dimention with the first dimension, and roll it back, change of dimension.
    #non-linearity activation function
    att = F.relu(att_wrapped)  #(batch_size * c_len, 1600)
    #         print("att", att.shape)
    c_mask = c_mask.unsqueeze(dim=2).float()  #(batch_size, c_len, 1)

    drop_att = F.dropout(att, self.drop_prob,
                         self.training)  #(batch_size * c_len, hidden_size)
    ...
    self_att = self.trilinear(
        encoder, encoder)  # get the self attention (batch_size, c_len, c_len)

    # to match the shape of the attention matrix
    mask = (c_mask.view(c_mask.shape[0], c_mask.shape[1], 1) *
            c_mask.view(c_mask.shape[0], 1, c_mask.shape[1]))
    identity = torch.eye(c_mask.shape[1],
                         c_mask.shape[1]).cuda().view(1, c_mask.shape[1],
                                                      c_mask.shape[1])
    ...
    conc = torch.cat((self_att_vector, encoder, encoder * self_att_vector),
                     dim=-1)
